...
extern ipc_handle ipcHndl_s1ap_attach_reject;

static struct attachReqRej_info *g_attachReqRejInfo;
...
init_stage()
{
	if ((ipcHndl_s1ap_attach_reject  = open_ipc_channel(
...
read_next_msg()
{
	...
	while (bytes_read < S1AP_REQ_REJECT_BUF_SIZE) {
		if ((bytes_read = read_ipc_channel(
				ipcHndl_s1ap_attach_reject , buf,
...
s1ap_attach_reject_processing()
{
	...
	g_attachReqRejInfo = (struct attachReqRej_info *) buf;
...
{
	close_ipc_channel(ipcHndl_s1ap_attach_reject);
	pthread_exit(NULL);
	return;
}
...
*/
void*
s1ap_attach_reject_handler(void *data)
{
	init_stage();
